...
const defaultQuery: OmniQueryValues = {query: 'https://steamcommunity.com/id/b4nny/'} // 76561197970669109
    ...
    const form = useForm({
        ...
        onSubmit: async (values) => {
            try {
                ...
                await navigate({to: '/profiles/$steamId', params: {steamId: steamID}})
            } catch (e) {
                    ...
                    children={(field) => {
                        return <>
                            <Input value={field.state.value} onBlur={field.handleBlur}
                                   onChange={(e) => field.handleChange(e.target.value)} type="query"
                                   placeholder="steamid"/>
